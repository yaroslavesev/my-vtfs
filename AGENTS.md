Я делаю лабу по ос, код для самой файловой системы уже написан. В папке lab/vtfs/server написан сервер 
на typeScript Нужно сделать то же самое только на спринг буте + котлине, я создал рядом lab/vtfs/os-server, 
он должен делать точно то же самое что и серверс на ts, чтобы можно было бесшовно запускать 
его и на виртуалке запускать фс. бд тоже в докере, без пгадмина и тд, обычный постгрес, без env переменных,
просто хардкод
после того как напишешь код нужно рассказать как запускать и что поднимать


вот тз, отсюда только про сервер интересно
# ЛР. Файловая система
