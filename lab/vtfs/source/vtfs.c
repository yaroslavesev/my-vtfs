...
static const char *sb_token(struct super_block *sb)
{
    if (sb && sb->s_fs_info)
        return (const char *)sb->s_fs_info;
    return "my_token";
}
...
static ssize_t vtfs_read(struct file *filp, char __user *buffer, size_t length, loff_t *offset)
{
    ...
    *offset += length;
...

    {
        ...
        ret_http = vtfs_http_call(sb_token(sb), "read", response, sizeof(response),
                                  2,
    ...
    *offset += length;
...

    {
        ...
        if (data_enc) {
            ...
            ret_http = vtfs_http_call(sb_token(sb), "write", response, sizeof(response),
                                      3,
                                      "parent_ino", parent_ino_str,
                                      "name", name_enc,
                                      "data", data_enc);
...
static struct dentry *vtfs_lookup(struct inode *parent_inode, struct dentry *child_dentry, unsigned int flag)
{
    ...
    if (file_info) {
        umode_t mode = file_info->is_dir ? (S_IFDIR | S_IRWXU | S_IRWXG | S_IRWXO)
                                         : (S_IFREG | S_IRWXU | S_IRWXG | S_IRWXO);

        struct inode *inode = vtfs_get_inode(parent_inode->i_sb, mode, file_info->ino);
        if (inode) {
            inode->i_op = parent_inode->i_op;
            inode->i_fop = file_info->is_dir ? parent_inode->i_fop : inode->i_fop;
                       ...
                       struct dentry *child_dentry, umode_t mode, bool b)
{
...

    {
        ...
        ret_http = vtfs_http_call(sb_token(parent_inode->i_sb), "create", response, sizeof(response),
                                  3,
                                  "parent_ino", parent_ino_str,
                                  "name", name_enc,
                                  "data", "");
...
static int vtfs_unlink(struct inode *parent_inode, struct dentry *child_dentry)
{
...

    {
        ...
        ret_http = vtfs_http_call(sb_token(parent_inode->i_sb), "unlink", response, sizeof(response),
                                  2,
                                  "parent_ino", parent_ino_str,
                                  "name", name_enc);
        if (ret_http < 0)
                      ...
                      struct dentry *child_dentry, umode_t mode)
{
    ...
    inode = vtfs_get_inode(parent_inode->i_sb, S_IFDIR | mode, next_ino++);
    if (!inode) {
        mutex_unlock(&vtfs_files_lock);
        return -ENOMEM;
    }
...

    {
        ...
        ret_http = vtfs_http_call(sb_token(parent_inode->i_sb), "mkdir", response, sizeof(response),
                                  2,
                                  "parent_ino", parent_ino_str,
                                  "name", name_enc);
        if (ret_http < 0)
...
static int vtfs_link(struct dentry *old_dentry, struct inode *parent_dir, struct dentry *new_dentry)
{
    ...
    {
        struct inode *new_inode = vtfs_get_inode(parent_dir->i_sb, old_inode->i_mode, new_file_info->ino);
        if (!new_inode)
            return -ENOMEM;
...
{
    const char *token = data ? (const char *)data : "my_token";
    sb->s_fs_info = kstrdup(token, GFP_KERNEL);
    if (!sb->s_fs_info)
        return -ENOMEM;
...
{
    struct dentry *ret = mount_nodev(fs_type, flags, (void *)dev_name, vtfs_fill_super);
    if (!ret)
